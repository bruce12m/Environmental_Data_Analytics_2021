...
#PSWID <- URL %>% html_nodes(PSWID_tag) %>% html_text()
#ownership <- URL %>% html_nodes(ownership_tag) %>% html_text()
#mmw <- URL %>% html_nodes(mmw_tag) %>% html_text()
...
thedf <- scrape(year, PSWID)
view(thedf)

thedf$Month <- factor(thedf$Month, levels=c('Jan', 'Feb', 'Mar', 'Apr', 
                                            'May', 'Jun', 'Jul', 'Aug', 
                                            'Sep', 'Oct', 'Nov', 'Dec'))

# Plot
ggplot(thedf,aes(x=Month,y=maximum_withdrawl_MGD)) + 
  geom_point() +
...
PSWID <- c('01-11-010','03-32-010')
year = 2015
#thedf2 <- scrape(year, PSWID)
#view(thedf2)

#thedf$Month <- factor(thedf2$Month, levels=c('Jan', 'Feb', 'Mar', 'Apr', 
#                                            'May', 'Jun', 'Jul', 'Aug', 
#                                            'Sep', 'Oct', 'Nov', 'Dec'))
...
dfGood <- cross2(year,PSWID) %>% 
  map(lift(scrape)) %>% 
  bind_rows()

dfsGood <- lapply(X = year,
                  FUN = scrape,
                  PSWID=PSWID)
dfGoodBig <- bind_rows(dfsGood)
...
#This is probably wrong here and is messing up the ggplot below 
thedf$Month <- factor(thedf$Month, levels=c('Jan', 'Feb', 'Mar', 'Apr', 
                                            'May', 'Jun', 'Jul', 'Aug', 
                                            'Sep', 'Oct', 'Nov', 'Dec'))
...
the_dfs <- lapply(X = year,
                  FUN = scrape,
                  PSWID=PSWID)
dfBig <- bind_rows(the_dfs)

ggplot(dfBig,aes(x=Date,y=maximum_withdrawl_MGD)) + 
  geom_point() +
...
The line of best fit indicates that, particularly over the past ~2 years, Asheville's water consumption has been increasing.