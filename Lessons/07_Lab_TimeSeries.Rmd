...
plot.rainfall.time # This one is probably stationary (not changing over time)
    
plot.temp.time <- 
  ggplot(BRA_complete, aes(x = Date, y = Temperature_C)) + 
  geom_line() + 
  geom_smooth(method = lm) # what about loess instead of lm?

plot.temp.time # This one is probably not stationary (it is changing over time)
...
temp_data_ts <- ts(BRA_complete$Temperature_C,
                   start=c(f_year,f_month),
                   frequency=12)
rain_data_ts <- ts(BRA_complete$Rainfall,
                   start=c(f_year,f_month),
                   frequency=12)
...
BRA.rain.components <- as.data.frame(rain_data_decomp$time.series[,1:3])
BRA.temp.components <- as.data.frame(temp_data_decomp$time.series[,1:3])

# Exercise 10: Visualize how the trend maps onto the data for both series
BRA.rain.components <- 
  mutate(BRA.rain.components, 
         Rainfall = BRA_complete$Rainfall, 
         Temperature_C = BRA_complete$Temperature_C, 
         Date = BRA_complete$Date)

BRA.rain.plot <- ggplot(BRA.rain.components) + 
...
# Run SMK tests
rain_data_trend1 <- Kendall::SeasonalMannKendall(ts_Rain)
rain_data_trend2 <- smk.test(ts_Rain)
temp_data_trend1 <- SeasonalMannKendall(ts_Temp)
temp_data_trend2 <- smk.test((ts_Temp))
...
ts_Rain_noseason <- ts(data = Rain_Components$Observed - Rain_Components$seasonal,
                       start = c(f_year,f_month),
                       frequency = 12)
#Apply Mann Kendall
...
Rain_Components <- mutate(Rain_Components, Rain_noseason = Observed - seasonal)
#plotting resulting series
ggplot(Temp_Components) +
  geom_line(aes(y = Observed, x = Date),  size = 0.25, alpha=0.3) +
  geom_line(aes(y = Temp_noseason, x = Date), color = "red") +
  geom_smooth(aes(y = Temp_noseason, x = Date)) +
  ylab("Temperature")
#plotting resulting series
ggplot(Rain_Components) +
  geom_line(aes(y = Observed, x = Date),  size = 0.25, alpha=0.3) +
  geom_line(aes(y = ts_Rain_noseason, x = Date), color = "green", alpha=0.7) +
