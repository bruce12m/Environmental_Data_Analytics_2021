...
# This is the old-school way of subsetting to just the 0 depth but you can also use filter
plot1 <- ggplot(filter(PeterPaul.chem.nutrients,depth == 0,],
                aes(x = daynum, 
                    y = temperature_C, 
                    color = year4)) + 
  geom_point(alpha = .5) + # alpha adjusts point transparency 
...
plot2 <- ggplot(PeterPaul.chem.nutrients, aes(x = sampledate, y = temperature_C, color = depth)) + geom_point(size = .5, alpha = .5) + scale_color_viridis(option = "magma", direction = -1)
print(plot2)
...
plot3 <- ggplot(EPAair,
                aes(x = Ozone, 
                    ...
                    color = SITE_LATITUDE)) +
  geom_point(alpha = .5) + # alpha adjusts point transparency
  geom_smooth(method = lm, se = FALSE, color = "orange") # does not go through origin 
print(plot3)
