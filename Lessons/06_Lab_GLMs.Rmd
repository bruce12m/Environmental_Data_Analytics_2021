...
> Answer: 
H0 (null): mu (average) >= 50 ppm of ozone 
Ha (alternative): mu < 50 ppm of ozone


```{r}

# Shapiro Wilk test can only run on 5000 records so we have to subset 
summary(EPAair$Ozone)
EPAair.subsample <- sample_n(EPAair, 5000)

# Evaluate assumption of normal distribution
# This p value is actually just telling us the likelihood that our data are normally distributed (we aren't yet testing OUR null hypothesis)
shapiro.test((EPAair.subsample$Ozone))
ggplot(EPAair, aes(x = Ozone)) +
  geom_histogram() 
qqnorm(EPAair$Ozone); qqline(EPAair$Ozone)
# not perfect normal distrubtion but we will proceed anyways

# And here is the T Test! 
O3.onesample <- t.test(EPAair$Ozone, mu = 50, alternative = "less")
O3.onesample
# mu and alternative in tandem tell the t test that our H0 is >= 50
...
print(Ozone.plot)
# That expand/scale x continuous function gets rid of this gap between the graph and the axis
```
...
> On average, ozone measured in NC is less than 50 ppm over x time period (how we substantiate this claim: t test, degrees of freedom = 6829, p value < 0.001)
...
```{r}
#Exercise 2:
ggplot(EPAair, aes(x = Ozone, fill=as.fact(Year))) +
...
library(corrplot)
EPA_Coor <- cor(EPAair %>% select(SITE_LATITUDE:PM2.5) %>% na.omit())
corrplot(EPA_Coor, method="ellipse")
corrplot.mixed(EPA_Coor, method="ellipse")
...
Ozone.regression <- lm(data=EPAair, PM2.5 ~ Ozone + Month + Year + SITE_LONGITUDE + SITE_LATITUDE)
...
the_best_model <- lm(formula = PM2.5 ~ Ozone + Month + SITE_LONGITUDE + SITE_LATITUDE, data=EPAair)
